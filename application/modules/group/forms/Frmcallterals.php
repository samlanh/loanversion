...
Class Group_Form_Frmcallterals extends Zend_Dojo_Form {
	...
	public function FrmCallTeral($data=null){
		...
		$_status_search->setAttribs(array('dojoType'=>$this->filter,'class'=>'fullside','autoComplete'=>"false",
    			'queryExpr'=>'*${0}*',));
		$_status_opt = array(
...
// 		$collteral_type->setValue($request->getParam('collteral_type'));
		...
		$to_date->setAttribs(array('dojoType'=>'dijit.form.DateTextBox',
				'constraints'=>"{datePattern:'dd/MM/yyyy'}",'required'=>'true','class'=>'fullside',
		));
		...
		$client_name->setAttribs(array('dojoType'=>'dijit.form.FilteringSelect','class'=>'fullside','autoComplete'=>"false",
    			'queryExpr'=>'*${0}*',));
