<?php
class Saving_CustomerController extends Zend_Controller_Action {
	...
	public function indexAction(){
		try{
			$db = new Saving_Model_DbTable_DbClient();
			...
			$collumns = array("BRANCH_NAME","CUSTOMER_CODE","CLIENTNAME_KH","CLIENTNAME_EN","SEX","PHONE","HOUSE","STREET","VILLAGE","SPOUSE_NAME",
					"DATE","BY_USER","STATUS");
			$link=array(
					'module'=>'saving','controller'=>'customer','action'=>'edit',
			);
			$link1=array(
					'module'=>'saving','controller'=>'customer','action'=>'edit',
			);
		...
		if($this->getRequest()->isPost()){
				...
				try{
				 if(isset($data['save_new'])){
					...
					$id= $db->addClient($data);
					Application_Form_FrmMessage::message("ការ​បញ្ចូល​ជោគ​ជ័យ !");
					Application_Form_FrmMessage::redirectUrl("/saving/customer");
				}
	...
	public function editAction(){
		...
		if($this->getRequest()->isPost()){
			try{
				$data = $this->getRequest()->getPost();
				$id= $db->addClient($data);
				Application_Form_FrmMessage::Sucessfull('EDIT_SUCCESS',"/saving/customer");
			}catch (Exception $e){
		...
		if(empty($row)){
			$this->_redirect("/saving/customer");
		}
		$fm = new Saving_Form_FrmClient();
