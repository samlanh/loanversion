...
class Callecterall_Model_DbTable_DbChangeCallecterall extends Zend_Db_Table_Abstract
{
   ...
   function getChenginfo($id){
   	$db = $this->getAdapter();
   	$sql="SELECT branch,receipt,code_call,customer_id,type_call,owner_call,
   	callnumber,create_date,term FROM ln_callecteralllist WHERE STATUS=1 AND code_call=$id LIMIT 1";
   	return $db->fetchAll($sql);
   }
   function getOwnerbyid($id){//ajax
   	$db = $this->getAdapter();
   	$sql = "SELECT* FROM ln_callecteralllist WHERE customer_id=$id LIMIT 1";
   	return $db->fetchRow($sql);
   }
}
